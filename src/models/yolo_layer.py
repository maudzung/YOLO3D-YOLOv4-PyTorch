...
class YoloLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size):
        ...
        self.scaled_anchors = torch.tensor(
            [(a_h / self.stride, a_w / self.stride, a_l / self.stride, im, re) for a_h, a_w, a_l, im, re in
        ...
        self.sa_polygons, self.sa_volumes, self.sa_low_hs, self.sa_high_hs = get_polygons_areas_fix_xyz(
            self.scaled_anchors)
    ...
    def build_targets(self, pred_boxes, pred_cls, target, anchors):
        ...
        if n_target_boxes > 0:  # Make sure that there is at least 1 box
            ...
            target_boxes = torch.cat((target[:, 2:4] * nG, target[:, 4:5], target[:, 5:8] * nG, target[:, 8:10]), dim=-1)

            gxyz = target_boxes[:, :3]
            ghwl = target_boxes[:, 3:6]
            gimre = target_boxes[:, 6:8]

            tg_polygons, tg_volumes, tg_low_hs, tg_high_hs = get_polygons_areas_fix_xyz(target_boxes[:, 3:8])
            # Get anchors with best iou
            ious_a_tg = iou_rotated_boxes_targets_vs_anchors(self.sa_polygons, self.sa_volumes, self.sa_low_hs,
        ...
        return iou_scores, giou_loss, class_mask, obj_mask, noobj_mask, tx, ty, tz, th, tw, tl, tim, tre, tcls, tconf
    ...
    def forward(self, x, targets=None, img_size=608, use_giou_loss=False):
        ...
        pred_boxes = torch.empty(prediction[..., :8].shape, device=self.device, dtype=torch.float)
        ...
        output = torch.cat((
            ...
            pred_boxes[..., 3:6].view(num_samples, -1, 3) * self.stride,  # h, w, l
